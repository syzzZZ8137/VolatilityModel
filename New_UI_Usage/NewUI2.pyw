...
def cal_surface():
    global in_put,contractChosen,daysChosen,temp_db,temp_db_buy,temp_db_sell,temp_db_ref,temp_db_ref_buy,temp_db_ref_sell
    #print(temp_db)
    res,res3D,interpoints,strike_lst,T_lst,benchmark = cal(temp_db)
    res_b,res3D_b,interpoints_b,strike_lst,T_lst,benchmark_b = cal(temp_db_buy)
    res_s,res3D_s,interpoints_s,strike_lst,T_lst,benchmark_s = cal(temp_db_sell)
    
    res_ref,res3D_ref,interpoints_ref,strike_lst,T_lst,benchmark_ref = cal(temp_db_ref)
    res_ref_b,res3D_ref_b,interpoints_b_ref,strike_lst,T_lst,benchmark_b_ref = cal(temp_db_ref_buy)
    res_ref_s,res3D_ref_s,interpoints_s_ref,strike_lst,T_lst,benchmark_s_ref = cal(temp_db_ref_sell)
    
    
    return res,res_b,res_s,res_ref,res_ref_b,res_ref_s,strike_lst,T_lst,benchmark['f_atm'].values[0],interpoints,interpoints_b,interpoints_s,res3D,res3D_b,\
res3D_s,res3D_ref,res3D_ref_b,res3D_ref_s,interpoints_ref,interpoints_b_ref,interpoints_s_ref
...
def drawPic_2D(sig):
    ...
    res,res_b,res_s,res_ref,res_ref_b,res_ref_s,strike_lst,T_lst,f_atm,interpoints,interpoints_b,interpoints_s,res3D,res3D_b,\
res3D_s,res3D_ref,res3D_ref_b,res3D_ref_s,interpoints_ref,interpoints_b_ref,interpoints_s_ref = cal_surface()
    #print(1)
...
def save(event):
    #update temp_db_ref
    global in_put,contractChosen,daysChosen,temp_db,temp_db_buy,temp_db_sell,param_list,temp_db_ref,temp_db_ref_buy,temp_db_ref_sell
    
    db_index = contractChosen.get()+'-'+daysChosen.get()
    ...
    temp_db_ref_buy,temp_db_ref_sell = BAspread(temp_db_ref,param_list)
    drawPic(0)

def Set():
    global temp_db,temp_db_buy,temp_db_sell,temp_db_ref,temp_db_ref_buy,temp_db_ref_sell
    temp_db = temp_db_ref.copy()
    temp_db_buy = temp_db_ref_buy.copy()
    temp_db_sell = temp_db_ref_sell.copy()
    drawPic(0)

def Revert():
    global temp_db,temp_db_buy,temp_db_sell,temp_db_ref,temp_db_ref_buy,temp_db_ref_sell
    temp_db_ref = temp_db.copy()
    temp_db_ref_buy = temp_db_buy.copy()
    temp_db_ref_sell = temp_db_sell.copy()
    drawPic(0)
    
    #把spinbox中的值变回原来的
    global in_put,contractChosen,daysChosen,param_list
    #tmp = data[0]['data']
...
def contact_day_to_params(event):
    #定义事件,该合约、天数---》参数更新
    global in_put,contractChosen,daysChosen,x_axischosen,temp_db,temp_db_buy,temp_db_sell,param_list,temp_db_ref
    #tmp = data[0]['data']
    ...
    temp_db_buy,temp_db_sell = BAspread(temp_db,param_list)
...
def contract_to_day(event):
    #定义事件,该合约--->更新天数
    global contractChosen,daysChosen,temp_db,labelbt6,var_lst,is_day,temp_db_ref
    temp_db_ref = 0
    data,bench_day = GetDataMySQL.getparamdata(exchange=contractChosen.get().split('-')[0],index=contractChosen.get().split('-')[-1],model='wing')
    #print(bench_day)
    temp_db = GetDataMySQL.get_std_paramdata(exchange=contractChosen.get().split('-')[0],index=contractChosen.get().split('-')[-1],model='wing')
    bench_day.sort()
...
labelframe1 = LabelFrame(label, text='Exchange-Future')
labelframe1.grid(row=2,column=1,sticky=N+W, padx=10,  pady=5)
contractChosen = ttk.Combobox(labelframe1, width=12)
contractChosen['values'] =  GetDataMySQL.get_future_info()  # 设置下拉列表的值
contractChosen.grid(column=1, row=3)      # 设置其在界面中出现的位置  column代表列   row 代表行
contractChosen.bind("<<ComboboxSelected>>",contract_to_day)


labelframe2 = LabelFrame(label, text='TimeToMaturity')
labelframe2.grid(row=4,column=1,sticky=N+W, padx=10,  pady=5)
daysChosen = ttk.Combobox(labelframe2, width=12)
daysChosen.grid(column=1, row=5)      # 设置其在界面中出现的位置  column代表列   row 代表行
daysChosen.bind("<<ComboboxSelected>>",contact_day_to_params)

labelframe3 = LabelFrame(label,text = 'X_axis')
labelframe3.grid(row=6,column=1,sticky=N+W,padx=10,pady=5)
x_axischosen = ttk.Combobox(labelframe3, width=12)
x_axischosen.grid(column=1, row=7)
